...
//                val allProducts = productsMap.values.toList().reversed()
    ...
    fun getAllProductsFlowPagination(startIndex: String?, pageSize: Int): Flow<ShopItems> = flow {
        val client = HttpClient(Darwin)

        try {
            // Paginate products using startAt and limitToFirst for the first batch of products.
            val url = if (startIndex != null) {
                "${FirebaseUtil.BASE_URL}product_master.json?orderBy=\"productId\"&startAt=\"$startIndex\"&limitToFirst=$pageSize"
            } else {
            ...
            if (response.status == HttpStatusCode.OK) {
                ...
                // Filter products directly on Firebase by querying for active products
                val validProducts = productsMap.values.filter { it.isActive == "0" && it.flag == "0" }

                // Now handle each product and fetch related data
                    ...
                    if (shopResponse.status == HttpStatusCode.OK) {
                        ...
                        if (shop != null && shop.isActive == "0" && shop.flag == "0") {
                            ...
                            if (imageResponse.status == HttpStatusCode.OK) {
                                ...
                                // Emit a single ShopItem after combining all data
                                val shopItem = ShopItems(
                                    image = imageUrl.orEmpty(),
                                    productId = product.productId,
                                    shopId = product.shopId,
                                ...
                                emit(shopItem) // Emit each product **one by one**
                                println("Emitting product ${product.productId}")
                            }
                        }
                    }
                }
